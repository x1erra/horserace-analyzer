                    ...
                    <table className="w-full text-left text-gray-300">
                        ...
                        <tbody className="divide-y divide-purple-900/20">
                            {currentItems.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="p-8 text-center text-gray-500 italic">No race results found.</td>
                                </tr>
                            ) : (
                                currentItems.map((result, index) => {
                                    const winnerStyle = result.winner_program_number ? getPostColor(result.winner_program_number) : null;
                                    return (
                                        <tr key={result.race_key || index} className="hover:bg-purple-900/10 transition duration-200 group">
                                            <td className="p-4 text-gray-300 font-medium whitespace-nowrap">{result.race_date}</td>
                                            <td className="p-4">
                                                <span className="inline-flex items-center justify-center px-2.5 py-1 rounded-md bg-purple-900/20 text-purple-300 text-xs font-bold border border-purple-500/30 font-mono">
                                                    {result.track_code || result.track_name}
                                                </span>
                                            </td>
                                            <td className="p-4 whitespace-nowrap">
                                                <Link
                                                    to={`/race/${result.race_key}`}
                                                ...
                                                <div className="flex items-center gap-3">
                                                    {winnerStyle ? (
                                                        <div
                                                            ...
                                                            style={{ backgroundColor: winnerStyle.bg, color: winnerStyle.text }}
                                                        >
                                                            {result.winner_program_number}
                                                        </div>
                                                    ) : (
